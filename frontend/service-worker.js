...
const fetchConfig = async () => {
  try {
    ...
    console.error("Defend SW /api/config fetch failed:", err);
    return null;
  }
};
...
const getAppVersion = async () => {
  ...
  console.log(`Defend SW app version: ${APP_VERSION}`);
  return APP_VERSION;
};
...
const installNewCache = async (version) => {
  ...
  console.log(`Defend SW installing cache ${cacheName}`);
  ...
  console.log(`Defend cache pre-populated for v${version}`);
};
...
const cleanupOldCaches = async (currentVersion) => {
  ...
  const deletes = names
    ...
    .map((name) => {
      console.log(`Defend cache deleted (old): ${name}`);
...
const checkAndUpdateCache = async () => {
  ...
  console.log(`Defend SW check: app=${appVersion} cache=${cacheVersion ?? "none"}`);
  if (!cacheVersion) {
...
self.addEventListener("install", (event) => {
  console.log(`Defend SW installing v${APP_VERSION}`);
  event.waitUntil(
    (async () => {
      try {
        await getAppVersion();
        ...
        console.error("Defend SW install failed:", err);
        throw err;
      }
...
self.addEventListener("activate", (event) => {
  console.log(`Defend SW activating v${APP_VERSION}`);
  event.waitUntil(
    (async () => {
