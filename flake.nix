{
  description = "Minimalist Nix-built container for Defend";
    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        backend = rustPlatform.buildRustPackage {
          ...
          installPhase = ''
            mkdir -p $out/bin
            cp target/release/backend $out/bin/defend-backend
        ...
        dockerImage = pkgs.dockerTools.buildLayeredImage {
          ...
          config = {
            Cmd = [ "${backend}/bin/defend-backend" ];
            WorkingDir = "/app";
            Env = [
              "PORT=4504"
            ];
            ...
            User = "65534:65534";
            Healthcheck = {
              Test = [ "CMD-SHELL" "wget -qO- http://localhost:4504/health >/dev/null 2>&1 || exit 1" ];
